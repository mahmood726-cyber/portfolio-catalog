"""Import-smoke + pure-function tests for portfolio-catalog (Bucket-B no-tests remediation).
...
_SCRIPT = os.path.join(
    ...
    'scripts', 'build_portfolio_catalog.py')


def _load():
    sys.path.insert(0, os.path.dirname(_SCRIPT))
    spec = importlib.util.spec_from_file_location("buildmod_portfolio_catalog", _SCRIPT)
